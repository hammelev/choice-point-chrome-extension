...
type uuidToRuleIdMapType = { [key: string]: number };
type blockedWebsitesListType = Array<{ uuid: string, url: string }>;
 ...
 */
const getStorageData = async (): Promise<{ desiredWebsites: blockedWebsitesListType, oldUuidToRuleIdMap: uuidToRuleIdMapType, nextRuleId: number }> => {
  try {
 ...
 * @param {blockedWebsitesListType} desiredWebsites - The desired list of blocked websites.
...
const calculateRuleChanges = (
  desiredWebsites: Array<{ uuid: string, url: string }>,
